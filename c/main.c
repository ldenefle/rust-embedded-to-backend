...
#define SAMPLE_MAX_LEN  20
#define PACKETS_MAX_LEN  50

struct CPacket {
    uint32_t timestamp;
    uint32_t stream_id;
    uint16_t sample_len;
    uint8_t sample[SAMPLE_MAX_LEN];
};

struct CParserResult {
    struct CPacket packets[PACKETS_MAX_LEN];
    uint16_t packets_len;
    uint32_t remaining;
};

extern int C_parse_packet(const uint8_t * bytes, const uint16_t bytes_len, struct CParserResult * result);

static const uint8_t very_valid_packet[] = { 0x60, 0x03, 0x00, 0x00, 0x0C, 0x51, 0xB2, 0x32, 0x5E, 0x00, 0x00, 0x02, 0x00, 0x88, 0xA1, 0xC1, 0x41, 0x0C, 0x92,
...
int main()
{
    struct CParserResult result;
    result.packets_len = 0;

    assert(C_parse_packet(NULL, sizeof(very_valid_packet), &result) == -1);
    assert(C_parse_packet(&very_valid_packet[0], sizeof(very_valid_packet), NULL) == -1);
    assert(C_parse_packet(&very_valid_packet[0], sizeof(very_valid_packet), &result) == 0);
    assert(result.packets_len == 39);
    assert(result.remaining == 0x360);
    assert(result.packets[0].sample_len == 4);
    assert(result.packets[0].timestamp ==  0x5E32B251);
    assert(result.packets[0].stream_id ==  0x20000);
    uint8_t first_sample[4] = { 0x88, 0xA1, 0xC1, 0x41 };
    assert(memcmp(result.packets[0].sample, &first_sample[0], sizeof(first_sample)) == 0);
